    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
      <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
        <h2 className="text-2xl font-black text-slate-800 mb-6 flex items-center gap-2">
          <Globe className="text-blue-600" /> 全球客户精准搜索
        </h2>
        ...
        <button 
          ...
          className="w-full mt-6 bg-slate-900 hover:bg-blue-600 text-white py-4 rounded-xl font-black shadow-lg transition-all flex items-center justify-center gap-2"
        >
      ...
      {state.hasSearched && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-black text-slate-800">搜索结果 ({state.results.length})</h3>
            <div className="flex gap-2">
              <button 
                disabled={selectedIndices.size === 0}
                onClick={() => onBatchAddToCRM(selectedResults)}
                className="flex items-center gap-2 bg-white border border-slate-200 px-4 py-2 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 disabled:opacity-50"
              >
              ...
              <button 
                ...
                className="flex items-center gap-2 bg-blue-600 px-4 py-2 rounded-xl text-sm font-bold text-white hover:bg-blue-700 disabled:opacity-50"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {state.results.map((res, idx) => (
              <div 
                key={idx} 
                className={`bg-white p-6 rounded-3xl border transition-all cursor-pointer ${selectedIndices.has(idx) ? 'border-blue-500 ring-2 ring-blue-100' : 'border-slate-200 hover:border-slate-300'}`}
