...
import { ModulePosterStudio } from './ModulePosterStudio';

type Tab = 'free' | 'poster';
...
export const ModuleImageGenerator: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 max-w-7xl mx-auto px-0">
        <button
          type="button"
          onClick={() => setTab('poster')}
          className={`flex-1 sm:flex-none px-4 py-3 rounded-xl text-sm font-black touch-manipulation ${
            tab === 'poster' ? 'bg-pink-600 text-white' : 'bg-white border border-slate-200 text-slate-600'
          }`}
        >
          产品海报（拍照/上传）
        </button>
        <button
          ...
          onClick={() => setTab('free')}
          className={`flex-1 sm:flex-none px-4 py-3 rounded-xl text-sm font-black touch-manipulation ${
            tab === 'free' ? 'bg-pink-600 text-white' : 'bg-white border border-slate-200 text-slate-600'
          }`}
        >
          自由文生图
        </button>
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  placeholder="例如：高清产品棚拍，白色背景，柔光，电商主图风格"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                ...
                <div>
                  <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">数量</label>
                  <select
                    value={count}
              ...
              <button
                ...
                className="w-full bg-pink-600 hover:bg-pink-700 text-white py-4 rounded-xl font-black shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50 touch-manipulation"
              >
              ...
              {error && (
                <div className="p-4 bg-red-50 border border-red-100 rounded-xl text-sm font-bold text-red-600 flex gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {images.map((url, idx) => (
                <div key={idx} className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                  <img src={url} alt={`gen-${idx}`} className="w-full object-cover" />
